...
contract TicketEscrow is Ownable, ReentrancyGuard {
    ...
    mapping(bytes32 => uint256[]) private _flightOrders;

    function getFlightOrders(
        bytes32 flightIdHash
    ) external view returns (uint256[] memory) {
        return _flightOrders[flightIdHash];
    }
    ...
    function createOrder(
        ...
        uint40 arrivalTs,
        bool refundOnCancel
    ) external nonReentrant returns (uint256 orderId, uint256 policyId) {
        ...
        _flightOrders[flightIdHash].push(orderId);

        // Ticket funds stay in escrow. Premium (if any) is forwarded to pool + policy minted.
        if (premium > 0) {
            require(
                policyManager != address(0) && insurancePool != address(0),
                "ESCROW:ADDR_NOT_SET"
            );
    ...
    function settleOrder(
        ...
        require(o.buyer != address(0), "ESCROW:NOT_FOUND");
        require(
            o.status == ORDER_CREATED || o.status == ORDER_POLICY_LINKED,
            "ESCROW:BAD_STATUS"
        );
        ...
        require(
            outcomeStatus == STATUS_ON_TIME ||
                outcomeStatus == STATUS_DELAYED ||
                outcomeStatus == STATUS_CANCELLED,
            "ESCROW:BAD_OUTCOME"
        );
